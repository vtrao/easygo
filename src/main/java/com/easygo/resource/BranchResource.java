...
public class BranchResource {
	public final IBranchDAO branchDAO;
	public BranchResource(IBranchDAO branchDAO) {
		super();
	...
	public Response addBranch(Branch branch, @Context HttpHeaders header, @Context HttpServletResponse response) {
		response.setHeader("Access-Control-Allow-Origin", "*"); 
		response.setHeader("Access-Control-Allow-Credentials","true"); 
		int status = 200;
		Branch returnValue = null;
		try {
			returnValue = branchDAO.addBranch(branch);
		} catch (Exception e) {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response updateCar(Branch branch, @Context HttpHeaders header, @Context HttpServletResponse response) {
		response.setHeader("Access-Control-Allow-Origin", "*"); 
		response.setHeader("Access-Control-Allow-Credentials","true"); 
		int status = 200;
		Branch returnValue = null;
		try {
			int updated = branchDAO.updateBranch(branch);
			returnValue = branch;
	...
	public Response delete(@PathParam("id") long id, 
		...
		try {
			if(branchDAO.removeBranch(id)<=0) {
